...
- 2026-06-16 Pyth result-aware exact-output launch-template slice:
  ...
  - Decision: add a single-pair current-Pyth `exact-output-results` route case to the template using the existing A/B pool placeholders. This keeps the active launch lane Pyth-only and avoids inventing a second live pool/token path. Three-hop Pyth result-aware planning remains covered in SDK tests; fresh live evidence for this new template route still needs a launch refresh.
