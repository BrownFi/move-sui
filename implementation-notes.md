...
- 2026-06-17 recipient-directed Pyth bundle zap wrapper parity slice:
  ...
  - Decision: add bundle-only transfer wrappers for `zap_in_a`, `zap_in_b`, `zap_out_a`, and `zap_out_b`. Zap-in wrappers send residual token coins back to `tx_context::sender` and LP to the requested recipient; zap-out wrappers send the single output coin to the requested recipient. Direct `OracleAdapter` zap wrappers remain return-based until a separate direct-compatibility need is proven.
