...
- Overflow bound tests. Coverage pins checked overflow behavior for low-decimal raw-to-standard amount scaling, `u256` to `u64` downcasts, a public Pyth bundle exact-input quote whose low-decimal input overflows standard normalization, a public Pyth bundle exact-output quote whose required input overflows raw token representability, and large exact-output inventory-penalty intermediate math; broader exact-input quadratic/intermediate and AMM arithmetic bounds remain pending.
