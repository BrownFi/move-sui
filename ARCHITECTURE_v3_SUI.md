...
Current-Pyth testnet launch evidence: the verifier-ready live evidence artifact at `configs/launch/pyth-current-testnet.live-evidence.matrix.json` records test coins package `0xff23e4c74d9fdc8820ea55ee8c79ea498d10e0b6a97ad1925d0e379989d3883c` in digest `bJHz72312R777BkUihaGMeRVoU9QaxB1fXC7UUEfpw2`, BrownFi package `0x26e904b6dd56a7f8ff1671584eb0b7da138993663e5d48294d3c2c004fe2a629` in digest `3n3WhX63AQ8SVjbhL8EcyGWpP2xSoX74BhjuSnPEYJEz`, pool `0xafbf7faf6b99f573a2717684451e9f9ecf9904cce6aa4b8cca1d75a730fe09ca` in digest `BFuhrzvjtgZKRvCNLrP7oNaSzFHiQy2hpXKWkQgnMjjz`, LP coin `0x3e933f5af65654f4792348d98e58cbec7ee1b20441edcf5dc68e38a3bfa4f590`, and flash enable digest `24jBprRwgqPG9aqcFAjZaeDHP6D86JjyDyW8t6EMWs6J`, each recheckable with `tools/verify-sui-cli-tx-evidence.mjs --setup <name>`. Package publish setup checks verify digest, successful effects, and published package ID through Sui object changes. Pool setup also verifies the pool object ID, LP coin ID, expected BrownFi pool-create call, and `Sync` / `PoolCreated` events. The checked route evidence covers exact-input swap `FhmphFaiT4RzHXfEprH4zGmZUtKDWSoeHT1x1u1jf9Ht`, exact-output swap `F6uVciswJ2SHphVExbrPRyeavmfmVegUFjpoC5diu1ga`, add liquidity `praGFhbZZFK7C7Aa3kWSsK9qWWBgriygZbdYBY6nuSE`, remove liquidity `GMb2P1Xhi9x3Y2NPc2t6wDzDZYodzhXyy33ATKiTZyDb`, zap-in-A `6qZzpQUT9ioZCPKbggQhMZwZ2Koh7AQuDR4SiJGiLXva`, zap-in-B `B24ehmKuXAyTTZRakE4DZ3YDRvX2hYAL2YoqXTJPwUhh`, zap-out-A `6Tw53jsQgbkBzgWMpknXrdrdntnSsMtUBod1Lr1gdzUr`, zap-out-B `ALnNCR5JfxFE6x5zTpiQZZfaTzgEMjE5wWw7PHKCCuNx`, flash-borrow-A `EQrn4T1LHaRLu7zuxKkTGsD2NGYHdW2NCapJa3onRvC9`, and flash-borrow-B `HiYzRpQUXRi4KTfn2DJMoaMmiyp7Q47WuyZmBeqwobuS`, each recheckable with `tools/verify-sui-cli-tx-evidence.mjs --tx <name>`. The same live-value-clean matrix also includes cutoff-aware and raw/no-cutoff exact-input/exact-output quote cases so quote coverage is validated alongside the landed route evidence; quote cases intentionally have no `txEvidence` because they are no-spend validation cases.
