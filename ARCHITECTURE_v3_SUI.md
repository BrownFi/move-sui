...
- Boundary tests for `kB`, `kQ`, `lambda`, `gamma`, fee, decimals, and reserves are covered by config/factory boundary tests, a Pyth bundle add-liquidity reserve-cap abort test, and a flash-fee cap abort test that prevents fee dust from pushing a repaid pool balance above the raw cap.
