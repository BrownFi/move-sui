...
- Overflow bound tests. Coverage pins checked overflow behavior for low-decimal raw-to-standard amount scaling, `u256` to `u64` downcasts, and a public Pyth bundle exact-output quote whose required input overflows raw token representability; broader exact-input swap-formula and AMM arithmetic bounds remain pending.
