...
module brownfi_amm::pool_test {
    ...
    fun test_add_liquidity_aborts_on_zero_input_a() {
        ...
        next_tx(&mut scenario, ADDR2);
        {
            ...
            let (remaining_a, remaining_b, lp) = swap::add_liquidity(&factory, &oracle, &pio_a, &pio_b, &clock, &mut pool, input_a, input_b, 0);
    ...
    fun test_add_liquidity_on_empty_pool() {
        ...
        next_tx(&mut scenario, ADDR1);
        {
            ...
            let factory = take_shared<Factory>(&scenario);
            let (a_out, b_out) = swap::remove_liquidity(&factory, &mut pool, coin::into_balance(lp_coin), 0, 0);
            return_shared(factory);
        ...
        next_tx(&mut scenario, ADDR2);
        {
            ...
            let (remaining_a, remaining_b, lp) = swap::add_liquidity(&factory, &oracle, &pio_a, &pio_b, &clock, &mut pool, input_a, input_b, 141);
    ...
    fun test_add_liquidity() {
        ...
        next_tx(&mut scenario, ADDR2);
        {
            ...
            let input_a = balance::create_for_testing<A>(200);
            let input_b = balance::create_for_testing<B>(100);
            let (remaining_a, remaining_b, lp) = swap::add_liquidity(&factory, &oracle, &pio_a, &pio_b, &clock, &mut pool, input_a, input_b, 140);
        ...
        next_tx(&mut scenario, ADDR2);
        {
            ...
            let (remaining_a, remaining_b, lp) = swap::add_liquidity(&factory, &oracle, &pio_a, &pio_b, &clock, &mut pool, input_a, input_b, 70);
    ...
    fun test_add_liquidity_aborts_on_min_lp_out() {
        ...
        next_tx(&mut scenario, ADDR2);
        {
            ...
            let (remaining_a, remaining_b, lp) = swap::add_liquidity(&factory, &oracle, &pio_a, &pio_b, &clock, &mut pool, input_a, input_b, 201);
    ...
    fun test_remove_liquidity_aborts_on_zero_input_lp() {
        ...
        next_tx(&mut scenario, ADDR1);
        {
            ...
            let lp = balance::zero();
            let factory = take_shared<Factory>(&scenario);
            let (a_out, b_out) = swap::remove_liquidity(&factory, &mut pool, lp, 0, 0);
            return_shared(factory);
    ...
    fun test_remove_liquidity() {
        ...
        next_tx(&mut scenario, ADDR1);
        {
            ...
            let factory = take_shared<Factory>(&scenario);
            let (a_out, b_out) = swap::remove_liquidity(&factory, &mut pool, lp_in, 36, 4);
            return_shared(factory);
        ...
        next_tx(&mut scenario, ADDR1);
        {
            ...
            let factory = take_shared<Factory>(&scenario);
            let (a_out, b_out) = swap::remove_liquidity(&factory, &mut pool, lp_in, 2, 0);
            return_shared(factory);
        ...
        next_tx(&mut scenario, ADDR1);
        {
            ...
            let factory = take_shared<Factory>(&scenario);
            let (a_out, b_out) = swap::remove_liquidity(&factory, &mut pool, lp_in, 62, 9);
            return_shared(factory);
    ...
    fun test_remove_liquidity_aborts_on_min_a_out() {
        ...
        next_tx(&mut scenario, ADDR1);
        {
            ...
            let factory = take_shared<Factory>(&scenario);
            let (a_out, b_out) = swap::remove_liquidity(&factory, &mut pool, lp_in, 51, 50);
            return_shared(factory);
    ...
    fun test_remove_liquidity_aborts_on_min_b_out() {
        ...
        next_tx(&mut scenario, ADDR1);
        {
            ...
            let lp_in = coin::into_balance(coin::split(&mut lp_coin, 50, ctx));
            let factory = take_shared<Factory>(&scenario);
            let (a_out, b_out) = swap::remove_liquidity(&factory, &mut pool, lp_in, 50, 51);
            return_shared(factory);
