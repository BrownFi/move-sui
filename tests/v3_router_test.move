...
module brownfi_amm::v3_router_test {
    ...
    #[test]
    fun test_router_swap_a_for_exact_b_direct_transfer_sends_output_to_recipient_and_refunds_sender() {
        let mut scenario = test_helpers::init_test_scenario(ADDR1);
        test_helpers::create_test_pool(&mut scenario, 1_000_000, 1_000_000);
        ...
        next_tx(&mut scenario, ADDR1);
        {
            ...
            router::swap_a_for_exact_b_and_transfer(
                &oracle,
                &pio_a,
                &pio_b,
                &clock,
        ...
        {
            let b_out = take_from_sender<Coin<B>>(&scenario);
            assert!(coin::value(&b_out) == 99_994, 1);
            balance::destroy_for_testing(coin::into_balance(b_out));
        };
    ...
    #[test]
    fun test_router_swap_b_for_exact_a_direct_transfer_sends_output_to_recipient_and_refunds_sender() {
        let mut scenario = test_helpers::init_test_scenario(ADDR1);
        test_helpers::create_test_pool(&mut scenario, 1_000_000, 1_000_000);
        ...
        next_tx(&mut scenario, ADDR1);
        {
            ...
            router::swap_b_for_exact_a_and_transfer(
                &oracle,
                &pio_a,
                &pio_b,
                &clock,
        ...
        {
            let a_out = take_from_sender<Coin<A>>(&scenario);
            assert!(coin::value(&a_out) == 99_994, 1);
            balance::destroy_for_testing(coin::into_balance(a_out));
        };
    ...
    #[test]
    fun test_router_swap_a_for_exact_c_via_b_with_bundles_transfer_sends_output_and_refunds_sender() {
        let mut scenario = test_helpers::init_test_scenario(ADDR1);
        create_pyth_test_route(&mut scenario);
        ...
        next_tx(&mut scenario, ADDR1);
        {
            ...
            router::swap_a_for_exact_c_via_b_with_bundles_and_transfer(
                &bundle_ab,
                &bundle_bc,
                &clock,
                &mut pool_ab,
    ...
    #[test]
    fun test_router_pyth_bundle_exact_input_route_quotes_are_monotonic_in_amount() {
        let mut scenario = test_helpers::init_test_scenario(ADDR1);
        create_pyth_test_route(&mut scenario);
    ...
    #[test]
    fun test_router_pyth_bundle_exact_output_route_quotes_are_monotonic_in_output() {
        let mut scenario = test_helpers::init_test_scenario(ADDR1);
        create_pyth_test_route(&mut scenario);
        ...
        next_tx(&mut scenario, ADDR2);
        {
            ...
            assert!(small_effective_c == 100_000, 0);
            assert!(large_effective_c == 200_000, 1);
    ...
    fun assert_pyth_mixed_decimal_route_state_sequence(
        ...
        next_tx(&mut scenario, ADDR2);
        {
            ...
            assert!(coin::value(&c_out) > 0, 12);
            assert!(coin::value(&a_out) > 0, 13);
    ...
    #[test]
    fun test_router_swap_c_for_exact_a_via_b_with_bundles_transfer_sends_output_and_refunds_sender() {
        let mut scenario = test_helpers::init_test_scenario(ADDR1);
        create_pyth_test_route(&mut scenario);
        ...
        next_tx(&mut scenario, ADDR1);
        {
            ...
            router::swap_c_for_exact_a_via_b_with_bundles_and_transfer(
                &bundle_ab,
                &bundle_bc,
                &clock,
                &mut pool_ab,
    ...
    #[test]
    #[expected_failure(abort_code = router::EInsufficientBAmount)]
    fun test_router_add_liquidity_with_bundle_with_min_deposits_rejects_unreachable_min_b() {
        let mut scenario = test_helpers::init_test_scenario(ADDR1);
        create_pyth_test_pool(&mut scenario);
        ...
        next_tx(&mut scenario, ADDR2);
        {
            ...
            let (remaining_a, remaining_b, lp) = router::add_liquidity_with_bundle_with_min_deposits(
                &bundle,
                &clock,
                &mut pool,
                input_a,
