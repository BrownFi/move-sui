...
module brownfi_amm::v3_swap_test {
    ...
    fun test_add_liquidity_with_bundle_rejects_reserve_above_cap() {
        let mut scenario = test_helpers::init_test_scenario(ADDR1);
        create_pyth_test_pool_with_amounts(
            &mut scenario,
            999_999_999_999_999_500,
            999_999_999_999_999_500
        );
    ...
    #[test]
    #[expected_failure(abort_code = swap::EInsufficientOutputAmount)]
    fun test_swap_a_for_exact_b_with_bundle_rejects_zero_standard_output() {
        let mut scenario = test_helpers::init_test_scenario(ADDR1);
        create_pyth_test_pool_with_amounts_and_decimals(
            &mut scenario,
            1_000_000_000,
    ...
    #[test]
    #[expected_failure(abort_code = swap::EInsufficientOutputAmount)]
    fun test_swap_b_for_exact_a_with_bundle_rejects_zero_standard_output() {
        let mut scenario = test_helpers::init_test_scenario(ADDR1);
        create_pyth_test_pool_with_amounts_and_decimals(
            &mut scenario,
            1_000_000_000_000_000_000,
    ...
    #[test]
    fun test_exact_input_pyth_bundle_scales_output_between_six_decimal_quote_and_twelve_decimal_base() {
        let mut scenario = test_helpers::init_test_scenario(ADDR1);
        create_pyth_test_pool_with_amounts_and_decimals(
            &mut scenario,
            10_000_000,
    ...
    #[test]
    fun test_exact_input_pyth_bundle_scales_output_between_twelve_decimal_quote_and_six_decimal_base() {
        let mut scenario = test_helpers::init_test_scenario(ADDR1);
        create_pyth_test_pool_with_amounts_and_decimals(
            &mut scenario,
            10_000_000_000_000,
        ...
        next_tx(&mut scenario, ADDR2);
        {
            ...
            let input_amount = 1_000_000_000_000;
            let expected_output = 998_945;
            let (amount_out, raw_output, cutoff_output) = swap::quote_a_for_b_with_bundle(
                &bundle,
                &clock,
            ...
            assert!(balance::value(&b_out) == expected_output, 1);
            assert!(amount_a == 11_000_000_000_000, 2);
            assert!(amount_b == 9_001_055, 3);
    ...
    #[test]
    fun test_exact_output_pyth_bundle_scales_required_input_between_six_decimal_quote_and_twelve_decimal_base() {
        let mut scenario = test_helpers::init_test_scenario(ADDR1);
        create_pyth_test_pool_with_amounts_and_decimals(
            &mut scenario,
            10_000_000,
        ...
        next_tx(&mut scenario, ADDR2);
        {
            ...
            let requested_output = 1_000_000_000_000;
            let expected_required_input = 1_001_056;
            let (required_input, effective_out) = swap::quote_a_for_exact_b_with_bundle(
                &bundle,
                &clock,
            ...
            assert!(balance::value(&b_out) == requested_output, 2);
            assert!(amount_a == 11_001_056, 3);
            assert!(amount_b == 9_000_000_000_000, 4);
    ...
    #[test]
    fun test_exact_output_pyth_bundle_scales_required_input_between_twelve_decimal_quote_and_six_decimal_base() {
        let mut scenario = test_helpers::init_test_scenario(ADDR1);
        create_pyth_test_pool_with_amounts_and_decimals(
            &mut scenario,
            10_000_000_000_000,
    ...
    #[test]
    fun test_pyth_bundle_quotes_remain_monotonic_after_state_sequence() {
        let mut scenario = test_helpers::init_test_scenario(ADDR1);
        create_pyth_test_pool_with_amounts(&mut scenario, 10_000_000_000, 10_000_000_000);
        ...
        next_tx(&mut scenario, ADDR2);
        {
            ...
            let b_out = swap::swap_a_for_b_with_bundle(
                ...
                &clock,
                &mut pool,
                balance::create_for_testing<A>(500_000_000),
                0
            );
            let a_out = swap::swap_b_for_a_with_bundle(
                ...
                &clock,
                &mut pool,
                balance::create_for_testing<B>(300_000_000),
                0
            );
            ...
            assert!(b_large_raw >= b_small_raw, 4);
            assert!(b_large_cutoff >= b_small_cutoff, 5);
            assert!(a_small_effective_out == 100_000_000, 6);
            assert!(a_large_effective_out == 200_000_000, 7);
            assert!(a_large_in >= a_small_in, 8);
            assert!(b_small_effective_out == 100_000_000, 9);
            assert!(b_large_effective_out == 200_000_000, 10);
            assert!(b_large_in >= b_small_in, 11);
    ...
    #[test]
    fun test_add_liquidity_pyth_bundle_mints_from_raw_representable_six_decimal_quote_twelve_decimal_base_deposit() {
        let mut scenario = test_helpers::init_test_scenario(ADDR1);
        create_pyth_test_pool_with_amounts_and_decimals(
            &mut scenario,
            10_000_000,
        ...
        next_tx(&mut scenario, ADDR2);
        {
            ...
            let input_a = balance::create_for_testing<A>(2_000_000);
            let input_b = balance::create_for_testing<B>(1_000_000_000_999);
            let (remaining_a, remaining_b, lp) = swap::add_liquidity_with_bundle(
                &bundle,
                &clock,
                &mut pool,
            ...
            assert!(balance::value(&lp) == 2_000_000_000, 0);
            assert!(balance::value(&remaining_a) == 1_000_000, 1);
            assert!(balance::value(&remaining_b) == 999, 2);
            assert!(amount_a == 11_000_000, 3);
            assert!(amount_b == 11_000_000_000_000, 4);
            assert!(lp_supply == 22_000_000_000, 5);
    ...
    #[test]
    fun test_add_liquidity_pyth_bundle_mints_from_raw_representable_twelve_decimal_quote_six_decimal_base_deposit() {
        let mut scenario = test_helpers::init_test_scenario(ADDR1);
        create_pyth_test_pool_with_amounts_and_decimals(
            &mut scenario,
            10_000_000_000_000,
        ...
        next_tx(&mut scenario, ADDR2);
        {
            ...
            let input_a = balance::create_for_testing<A>(1_000_000_001_999);
            let input_b = balance::create_for_testing<B>(2_000_000);
            let (remaining_a, remaining_b, lp) = swap::add_liquidity_with_bundle(
                &bundle,
                &clock,
            ...
            assert!(balance::value(&lp) == 2_000_000_000, 0);
            assert!(balance::value(&remaining_a) == 1_999, 1);
            assert!(balance::value(&remaining_b) == 1_000_000, 2);
            assert!(amount_a == 11_000_000_000_000, 3);
            assert!(amount_b == 11_000_000, 4);
            assert!(lp_supply == 22_000_000_000, 5);
